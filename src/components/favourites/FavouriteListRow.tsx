...
export const FavouriteListRow: React.FC<{ recipe: FavouriteItem; onRemove: (id: string) => void }> = ({ recipe, onRemove }) => {
  ...
  return (
    ...
    >
      <div className="relative w-28 h-24 flex-shrink-0 overflow-hidden">
        ...
        <span
          className="absolute top-2 left-2 px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wide text-white bg-[#E8500B]"
        >
          {recipe.category}
        </span>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between min-w-0">
        <div>
          ...
          <p className="mt-1 text-sm text-[#1a1a1a] opacity-60 line-clamp-1">{recipe.category} · TheMealDB</p>
        </div>
        <div className="flex items-center gap-4 text-xs text-[#1a1a1a] opacity-70 mt-2">
          ...
          <span className="text-xs text-[#1a1a1a] opacity-50">{new Date(recipe.savedAt).toLocaleDateString()}</span>
        </div>
      </div>
